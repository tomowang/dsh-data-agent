...
- The Settings panel's raw HTTP routes use a hand-rolled trust check rather than the harness's own `/api` trust fence (which is specific to `ctx.remote` calls). The `Host` header must name this server (loopback, or this machine's own addresses on a `0.0.0.0` bind), which defeats DNS rebinding. `Origin`, when sent, must match it. Every API route requires a JSON POST, which a cross-site page can't send without a CORS preflight. There's still no authentication: any local process can call these routes, which fits the loopback-only threat model but doesn't match the harness's own guarantees.
