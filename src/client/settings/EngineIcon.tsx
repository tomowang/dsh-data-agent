 ...
 * `default` dolphin-only mark reads as a blank blob at 16px), PostgreSQL and
 * SQLite render their `default` full-color logo. Only `height` is set —
 * these variants aren't all square (the MySQL wordmark and SQLite mark are
 * both wider than tall), so `width` is left to scale from each SVG's own
 * `viewBox` aspect ratio instead of being forced to `size` and distorted.
 */
